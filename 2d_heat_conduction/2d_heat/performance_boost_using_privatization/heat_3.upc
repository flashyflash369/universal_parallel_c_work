...
int main(void)
{
    //Declaration of the new private pointers

    double *ptr_priv[(N+2)/2];
    double *new_ptr_priv[(N+2)/2];
    //End Declaration of private pointers
    ...
    {
        ptr[i] = &grid[i][0];
        new_ptr[i] = &new_grid[j][0];

        ptr_priv[i] = $grid[i][0];
        new_ptr_priv[i] = &new_grid[i][0];
    }
    /* ----------------------------------------------------------------- */
