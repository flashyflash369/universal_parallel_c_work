...
shared [(N+2)* (N + 2) * THREADS] double grid[(N+2) * THREADS][(N+2) * THREADS], new_grid[(N+2) * THREADS][(N+2) * THREADS];
...
shared [(N+2) * (N + 2) * THREADS] double *ptr[(N + 2) * THREADS], *new_ptr[(N + 2) * THREADS], *tmp_ptr;
/* ----------------------------------------------------------------- */
...
int main(void)
{
    ...
    {
        dTmax = 0.0;
        upc_forall( i=1; i<(N+2)*THREADS - 1; i++; i/(N+2) )
        {
            for( j=1; j<(N+2)*THREADS - 1; j++ )
            {
                /* -------------------------------------------------------- */
